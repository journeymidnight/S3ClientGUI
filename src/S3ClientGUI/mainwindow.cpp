...
void MainWindow::OnUploadDir()
{
    ...
    for (auto tmpFile : file_list) {
        QString localFile = tmpFile.absoluteFilePath();
        ...
        addTask(remoteBucketName, localFile, KeyName, tmp, true);
    }
}
...
void MainWindow::OnDownloadDir()
{
    ...
    connect(loAction, &ListObjectAction::ListObjectFinished, this, [=](bool success, s3error error,
        ...
        for (auto tmpFile : downloadList) {
            qDebug() << "fileName" << m_fsview->currentPath() << endl;
            qDebug() << "bucket=" << bucketName << endl;
            qDebug() << "keyName=" << tmpFile << endl;
            QString tmpKeyName = tmpFile;
            QString currentPrefix = m_s3model->getCurrentPrefix();
            ...
            QStringList strList = downloadFile.split('/');
            tmp.chop(strList.at(strList.size() - 1).length());
            ...
            addTask(item->bucketName, downloadFile, tmpFile, localKey, false);
        }
        downloadList.clear();
    });
}
